...
def _get_time_limit(env, max_episode_length):
    """Taken from https://github.com/rlworkgroup/garage in its
    majority, only changed nomenclature for bug-fixing purposes.
    ...
    if hasattr(env, 'spec') and env.spec and hasattr(env.spec,
                                                     'max_episode_length'):
        spec_steps = env.spec.max_episode_length
    elif hasattr(env, '_max_episode_length'):
        spec_steps = getattr(env, '_max_episode_length')
...
class GymEnv(Environment):
    """Taken from https://github.com/rlworkgroup/garage in its
    majority, removed certain functions useless for the 
